...
namespace devspace\privacypolicy\core;
...
class privacypolicy
{
	...
	public function display_privacy_data($user_id)
	{
		...
		foreach ($template_array as $key => $data)
		{
			...
			{
				$this->template->assign_block_vars('cpf_data', [
					'FIELD_NAME' => $data[0],
					'FIELD_DATA' => $data[1],
				]);
			}
		 ...
		 *
		 * @event devspace.privacypolicy.add_data_after
		 *
		...
		extract($this->dispatcher->trigger_event('devspace.privacypolicy.add_data_after', compact($vars)));
	...
	public function create_csv($username, $user_id)
	{
		 ...
		 *
		 * @event devspace.privacypolicy.add_csv_data_after
		 *
		...
		extract($this->dispatcher->trigger_event('devspace.privacypolicy.add_csv_data_after', compact($vars)));
		 ...
		 * Event to allow adding additional user's privacy data to the CSV file
		 *
		 * @event devspace.privacypolicy.add_csv_cpf_after
		 *
		 * @var  string  csv_header  The CSV header row
		 * @var  string  csv_data    The CSV data row
		 *
		...
		extract($this->dispatcher->trigger_event('devspace.privacypolicy.add_csv_cpf_after', compact($vars)));
		 ...
		 * Event to allow adding additional user's privacy data to the CSV file
		 *
		 * @event devspace.privacypolicy.add_csv_ip_after
		 *
		 * @var  string  csv_header  The CSV header row
		 * @var  string  csv_data    The CSV data row
		 *
		...
		extract($this->dispatcher->trigger_event('devspace.privacypolicy.add_csv_ip_after', compact($vars)));
	...
	public function get_cpf_user_data($user_id)
	{
		$sql = $this->db->sql_build_query('SELECT', [
			...
			'LEFT_JOIN' => [
				[
					'FROM' => [$this->tables['profile_fields_data'] => ' pfd', ],
	...
	public function get_cpf_data($field_value, $field_name)
	{
		...
		$sql = $this->db->sql_build_query('SELECT', [
			...
			'LEFT_JOIN' => [
				[
					'FROM' => [$this->tables['profile_fields_options_language'] => ' pfl', ],
	...
	public function update_auto_groups($user_id, $sync = false)
	{
		...
		if ($this->autogroup_manager !== null)
		{
			if (!$sync)
			{
				...
				$this->autogroup_manager->check_condition('devspace.privacypolicy.autogroups.type.ppaccept', [
					'users' => $user_id,
				]);
			}
			else
