...
namespace devspace\privacypolicy\event;
...
class listener implements EventSubscriberInterface
{
	 ...
	 *
	 * @return \devspace\privacypolicy\event\listener
	 * @access public
	 */
	...
	public function page_footer($event)
	{
		$this->template->assign_vars([
			...
			'U_COOKIE_PAGE' 		=> $this->helper->route('devspace_privacypolicy_policyoutput', ['name' => 'policy']),
		]);
	...
	public function add_permissions($event)
	{
		...
		$permissions['a_privacy_view'] = ['lang' => 'ACL_A_PRIVACY_VIEW', 'cat' => 'misc'];
		$permissions['u_privacy_view'] = ['lang' => 'ACL_U_PRIVACY_VIEW', 'cat' => 'profile'];
	...
	public function check_cookie($event)
	{
		if ($this->config['cookie_policy_enable'])
		{
			...
			{
				$this->template->assign_vars([
					'U_REGISTER' 		=> $this->helper->route('devspace_privacypolicy_policyoutput', ['name' => 'access']),
					'U_LOGIN_LOGOUT'	=> $this->helper->route('devspace_privacypolicy_policyoutput', ['name' => 'access']),
				]);
			}
	...
	public function privacy_redirect($event)
	{
		...
		{
			redirect($this->helper->route('devspace_privacypolicy_acceptance'));
		}
	}
	...
	public function add_template_variables($event)
	{
		...
		if ($event['step'] == 1)
		{
			$this->template->assign_vars(['S_FIELD_PRIVACY_SHOW' => ($field_data['field_privacy_show']) ? true : false]);
		}
	}
